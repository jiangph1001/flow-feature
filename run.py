...
def read_pcap(pcapname,csvname):
    pcapsplit = pcapname.split("_")
    # 拆分文件名，获取App名称和行为
    if len(pcapsplit) == 5:
        app_name = pcapsplit[1]
        app_behave = pcapsplit[3]
    else:
        app_name = "Unknown"
        app_behave = "Unknown"
    print(pcapname,app_name,app_behave)
    try:
        # 可能存在格式错误读取失败的情况
    ...
    with open(csvname,"a+",newline="") as file:
        ...
        for v in streams.values():
            writer.writerow((v.start_time,v.end_time-v.start_time,v.src,v.sport,v.dst,v.dport,
                             app_name,app_behave,v.packet_num,v.byte_num,v.byte_num/v.packet_num,
                             v.protol))
            print(v)
